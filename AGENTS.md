...
App: `src/jobs/web.py`, `src/jobs/db.py`, `src/jobs/static/`. React source: `frontend/views/` and `frontend/components/`; tests: `frontend/tests/`. Node tooling and dependencies stay in `frontend/`. After React edits, run `npm --prefix frontend run build` and commit the rebuilt local `src/jobs/static/app.js` bundle. Run `npm --prefix frontend test` and relevant `npm --prefix frontend run test:browser` checks; browser tests use sample data. Agent scripts: `agent_scripts/`. Instructions stay here. The app and scripts share `jobs.db.DB_PATH`, resolved through platformdirs for `ai-jobs` (macOS: `~/Library/Application Support/ai-jobs/jobs.sqlite`). Never derive the live database path from the checkout or working directory. Schema upgrades must preserve all existing data. Keep user/, imports/, data/ and every apikey.txt ignored by Git. Don't delete private files during cleanup.

States: review (default), rejected, applied, interviewing. The one-off legacy status migration is complete; do not reintroduce it into app startup. Job actions are contextual: review → reject/apply; rejected → apply; applied → reject/interviewing; interviewing → reject/back to applied. Run `uv run python -m unittest discover -s tests -v` after backend changes. Apply marks applied and opens the listing; it doesn't submit an application. Retain the first applied_at after corrections. Bind only to 127.0.0.1. No profiles, frontend dev server or embedded agent. React uses a small esbuild step for development; normal Flask startup requires no Node or network assets.