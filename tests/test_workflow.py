...
class ReedTests(unittest.TestCase):
    def test_search_filters_dates_and_limits_output(self):
        ...
        today = date.today()
        rows = [dict(jobId=i, jobTitle='Analyst', employerName='Example', locationName='London',
        ...
        with tempfile.TemporaryDirectory() as folder, patch.object(reed, 'CACHE', Path(folder) / 'reed_jobs.json'), patch.object(search, 'get', return_value={'results': rows}), patch.object(sys, 'argv', ['search', '--keywords', 'finance', '--location', 'London', '--days', '7', '--limit', '20']):
            output = io.StringIO()
